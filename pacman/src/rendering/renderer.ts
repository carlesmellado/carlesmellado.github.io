...
import {
  ...
  PALETTE,
  Direction,
  FRIGHTENED_FLASH_PERIOD,
  READY_FRAMES,
} from '../constants/index.js';
import { TileMap, TileType } from '../grid/tilemap.js';
import { Pacman } from '../entities/pacman.js';
import { Ghost } from '../entities/ghost.js';
import { GhostName } from '../constants/index.js';
...
export interface RenderState {
  ...
  lives: number;
  level: number;
  frightenedFrames: number;
  frightenedFlashOut: number; // frames before flash phase starts
  paused: boolean;
  overlay: '' | 'READY!' | 'GAME OVER' | 'PAUSED';
  overlaySub: string;
  overlayFrames: number;
  mazeFlash: boolean;
  /** HUD label when the demo autopilot is driving. */
  auto: string;
  fruitVisible: boolean;
  fruitX: number;
  fruitY: number;
  popups: { x: number; y: number; text: string; frames: number }[];
}
...
export class Renderer {
  ...
  draw(map: TileMap, pacman: Pacman, ghosts: Record<GhostName, Ghost>, state: RenderState, frame: number): void {
    const c = this.ctx;
    c.fillStyle = PALETTE.background;
    c.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);

    this.drawMaze(map, frame, state.mazeFlash);
    if (state.fruitVisible) this.drawFruit(state.fruitX, state.fruitY);
    for (const p of state.popups) this.drawPopup(p);

    if (!pacman.dying) this.drawPacman(pacman);
    else this.drawDeath(pacman);

    for (const name of Object.keys(ghosts) as GhostName[]) {
      this.drawGhost(ghosts[name], state, frame);
    }

    this.drawHud(state);
    if (state.overlay) this.drawOverlay(state);
  }

  // ─── Maze ─────────────────────────────────────────────────────────────────

  private drawMaze(map: TileMap, frame: number, flash = false): void {
    const c = this.ctx;
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
      for (let col = 0; col < GRID_COLS; col++) {
        ...
        if (t === TileType.WALL) {
          ...
          c.lineWidth = 2;
          c.beginPath();
          if (!this.isEdge(map, col, row - 1)) { c.moveTo(x + 1, y + 1); c.lineTo(x + TILE_SIZE - 1, y + 1); }
          if (!this.isEdge(map, col, row + 1)) { c.moveTo(x + 1, y + TILE_SIZE - 1); c.lineTo(x + TILE_SIZE - 1, y + TILE_SIZE - 1); }
          if (!this.isEdge(map, col - 1, row)) { c.moveTo(x + 1, y + 1); c.lineTo(x + 1, y + TILE_SIZE - 1); }
          if (!this.isEdge(map, col + 1, row)) { c.moveTo(x + TILE_SIZE - 1, y + 1); c.lineTo(x + TILE_SIZE - 1, y + TILE_SIZE - 1); }
          c.stroke();
  ...
  private isEdge(map: TileMap, col: number, row: number): boolean {
    const t = map.tileAt(col, row);
  ...
  private drawPacman(p: Pacman): void {
    const c = this.ctx;
  ...
  private drawDeath(p: Pacman): void {
    const c = this.ctx;
  ...
  private drawGhost(g: Ghost, state: RenderState, frame: number): void {
    const c = this.ctx;
    const flashing =
      g.frightened &&
    ...
    if (g.frightened && g.stage !== 'eyes') body = flashing ? PALETTE.frightenedFlash : PALETTE.frightened;
  ...
  private drawFruit(x: number, y: number): void {
    const c = this.ctx;
    c.fillStyle = '#ff0000';
    c.beginPath();
  ...
  private drawPopup(p: { x: number; y: number; text: string; frames: number }): void {
    const c = this.ctx;
  ...
  static readyVisible(framesLeft: number): boolean {
