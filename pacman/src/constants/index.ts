...
export const DIRECTION_PRIORITY: readonly Direction[] = ALL_DIRECTIONS;
...
export const MAZE_LAYOUT: readonly string[] = [
  '                            ', //  0 HUD
  '                            ', //  1 HUD
  '                            ', //  2 HUD
  '############################', //  3
 ...
 */
export const NO_UP_TURN_TILES: readonly { col: number; row: number }[] = [
...
export const TUNNEL_SLOW_COLS = { leftMax: 5, rightMin: 22 } as const;
...
const pxPerFrame = (pct: number) => (BASE_SPEED_PX_PER_SEC * pct) / FPS;
...
export const ELROY_THRESHOLDS = { elroy1: 20, elroy2: 10 } as const;

/** Which SPEEDS keys back each Elroy tier (kept explicit so ghost AI is typed). */
export const ELROY_SPEED_KEYS = { elroy1: 'ghostElroy1', elroy2: 'ghostElroy2' } as const;

/** Pixels before a tile center within which Pac-Man may pre-turn (spec B). */
export const PRETURN_DISTANCE_PX = 4;

// ─── Global Wave Timers (spec D, frame-accurate @60Hz) ──────────────────────
export type WavePhase = { mode: 'SCATTER' | 'CHASE'; frames: number };

const SEC = FPS;
/** Wave 1 schedule: 7s scatter, 20s chase, 7s, 20s, 5s, 20s, 5s, chase forever. */
export const WAVE_SCHEDULE: readonly WavePhase[] = [
...
export const FRIGHTENED_FRAMES_BY_LEVEL: readonly number[] = [
...
export const SCORING = {
  dot: 10,
  energizer: 50,
  ghostCascade: [200, 400, 800, 1600] as readonly number[],
  extraLifeAt: 10_000,
} as const;
...
export const FRUIT = {
  ...
  spawnAfterDots: [70, 170] as readonly number[],
  /** ...and disappears after ~9.5 s. */
  lifetimeFrames: Math.round(9.5 * SEC),
  /** Point values by level (index = level-1, clamped). */
  pointsByLevel: [100, 300, 500, 700, 1000, 2000, 3000, 5000] as readonly number[],
} as const;
...
export const GHOST_RELEASE_FRAMES = [0, 3 * SEC, 6 * SEC, 9 * SEC] as const;
