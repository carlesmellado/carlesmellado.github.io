 ...
 */
import { Direction, DIR_VECTORS, TILE_SIZE, SPEEDS, GHOST_SPAWNS_PX, SCATTER_TARGETS, HOUSE_BOUNCE_PX, GHOST_DOOR_TILE, TUNNEL_SLOW_COLS, ELROY_THRESHOLDS, ELROY_SPEED_KEYS, } from '../constants/index.js';
...
const NAME_HASH = { blinky: 11, pinky: 23, inky: 37, clyde: 41 };
export class Ghost extends Entity {
    ...
    reset(levelStartFrame) {
        ...
        if (this.name === 'blinky') {
            ...
            const order = { pinky: 1, inky: 2, clyde: 3 };
    ...
    forceReverse() {
        if (this.stage !== 'out' || this.isEaten)
            return;
    ...
    roamingSpeed(world) {
        if (this.stage === 'eyes')
            return SPEEDS.ghostEyes;
        const { col } = this.tile();
        const inTunnel = this.tileMap.isTunnelRow(this.tile().row) &&
            (col <= TUNNEL_SLOW_COLS.leftMax || col >= TUNNEL_SLOW_COLS.rightMin);
        if (inTunnel)
            return SPEEDS.ghostTunnel;
        if (this.frightened)
            return SPEEDS.ghostFrightened;
        if (this.name === 'blinky') {
            if (world.remainingDots <= ELROY_THRESHOLDS.elroy2)
                return SPEEDS[ELROY_SPEED_KEYS.elroy2];
            if (world.remainingDots <= ELROY_THRESHOLDS.elroy1)
                return SPEEDS[ELROY_SPEED_KEYS.elroy1];
        }
    ...
    decide(ghost) {
        ...
        if (!this.tileMap.isWalkableByGhost(col, row))
            return false;
        ...
        if (ghost.frightened) {
            ...
            if (legal.length > 2) {
                if (options.length === 0)
                    options = [back]; // dead end: forced reverse
            }
            else {
            ...
            const pick = Math.floor(mulberry32(world.frame * 31 + NAME_HASH[ghost.name] + col * 7 + row) * options.length);
        ...
        if (options.length === 0)
            options = [back]; // dead end: forced reverse, never freeze
        ghost.dir = this.pickByTarget(options, world, ghost);
        return true;
    }
    /** Minimal Euclidean distance to target tile; ties follow ALL_DIRECTIONS order. */
    pickByTarget(options, world, ghost) {
        ...
        for (const dir of options) {
            ...
            const cand = { col: here.col + v.x, row: here.row + v.y };
    ...
    targetTile(world) {
        ...
        switch (this.name) {
            ...
            case 'inky': {
                ...
                if (pacDir === Direction.UP)
                    col -= 2; // same overflow quirk at ×2
